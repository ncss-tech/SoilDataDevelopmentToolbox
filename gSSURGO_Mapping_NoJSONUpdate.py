...
def ColorRamp(count, items, upperColor, lowerColor):
    ...
    try:
        ...
        for i in range(len(redList)):
            dColors[i] = (redList[i], greenList[i], blueList[i])
...
def CreateMapLayer(inputLayer, outputTbl, outputLayer, outputLayerFile, outputValues, parameterString):
    ...
    try:
        ...
        # 1, 3, 6, 2I
        #
        if dSDV["maplegendkey"] in [1, 3, 6] or (dSDV["maplegendkey"] == 2 and dSDV["attributelogicaldatatype"].lower() in ["integer", "float"]):
            try:
                #
        ...
        # will be ignored!!!
        #
        elif dSDV["maplegendkey"] in [4, 5, 7] or (dSDV["maplegendkey"] == 2 and dSDV["attributelogicaldatatype"].lower() in ["choice", "string"]):
...
def GetSDVAtts(gdb, sdvAtt):
    ...
    try:
        ...
        if dSDV["sqlwhereclause"] is not None:
            ...
            dSDV["sqlwhereclause"] = "UPPER(" + '"' + sqlParts[0].upper() + '") = ' + sqlParts[1].upper()
    ...
    try:
        ...
        if dSDV["effectivelogicaldatatype"].lower() in ["integer", "float"]:
            ...
            if iMin == None and iMax == -999999999:
                # No data
                raise MyError, "No numeric data for " + sdvAtt
...
try:
    ...
    drawOutlines = False                          # Polygon outlines are not drawn
            ...
            for rec in cur:
                ...
                if rtabphyname.upper() == dSDV["attributetablename"].upper():
                    ...
                    if rtabphyname == "MAPUNIT" and aggMethod != "No Aggregation Necessary":
                        ...
                        dMapunit = ReadTable(os.path.join(gdb, rtabphyname), flds, primSQL, level, sql)

                        if len(dMapunit) == 0:
                            raise MyError, "No mapunit level data for " + sdvAtt
                        ...
                        dComponent = ReadTable(os.path.join(gdb, rtabphyname), flds, primSQL, level, sql)

                        if len(dComponent) == 0:
                            raise MyError, "No component level data for " + sdvAtt
                        ...
                        #PrintMsg(" \nhzQuery: " + hzQuery, 1)
                        dHorizon = ReadTable(os.path.join(gdb, rtabphyname), flds, hzQuery, level, sql)
                        ...
                        dTbl = ReadTable(os.path.join(gdb, dSDV["attributetablename"].upper()), flds, primSQL, level, sql)
                    ...
                    if rtabphyname == "MAPUNIT":
                        ...
                        dMonth = ReadTable(os.path.join(gdb, rtabphyname), flds, primSQL, level, sql)
