# WSSThematic_gSSURGO.py
#
# ArcGIS 10.1
#
# Convert WSS Thematic Map Download to gSSURGO. Primary difference is the naming convention
# for the custom AOI folder.
#
# Adapted from SSURGO_Convert_to_Geodatabase.py
#
## ===================================================================================
...
def GetTemplateDate(newDB, areaSym):
    ...
    try:
        if not arcpy.Exists(newDB):
            raise MyError, "Unable to find database " + newDB
        ...
        if arcpy.Exists(saCatalog):
            ...
            raise MyError, "Unable to open " + saCatalog + " table in " + newDB
...
## ===================================================================================
def ImportTabular(newDB, tabularFolder, dbVersion, codePage):
    ...
    try:
        ...
        #newFolder = os.path.dirname(os.path.dirname(dbPath)) # survey dataset folder
        ...
        for txtFile in txtFiles:
            ...
            arcpy.SetProgressorLabel("Importing  tabular:   " + tbl)
        ...
        if dbDate == 0:
            ...
            raise MyError, "Failed to import tabular data, dbDate = 0"
        ...
        arcpy.AddIndex_management(os.path.join(newDB, "COINTERP"), "RULEDEPTH", "Indx_CointerpRuleDepth")
        arcpy.AddIndex_management(os.path.join(newDB, "COINTERP"), "MRULENAME", "Indx_CointerpMRuleName")
...
## ===================================================================================
def AppendFeatures(outputWS, AOI, mupolyList, mulineList, mupointList, sflineList, sfpointList, featCnt):
        ...
        if not procDates is None:
            ...
            for child in procDates.iter('procdate'):
                ...
                if sDate.find('xxTODAYxx'):
                    #PrintMsg("\Replacing process date: " + sDate + " with " + today, 1)
                    child.text = today
...
## ===================================================================================
def gSSURGO(inputFolder, outputWS, AOI, tileInfo, useTextFiles):
    # main function
    ...
    try:
        ...
        # example WSS Thematic folder:  wss_aoi_2017-07-03_11-32-36
        # all shapefile use this naming convention and there is no sapolygon featureclass unless I dissolve it from soilmu_a_aoi.shp
        # soilmu_a_aoi.shp
        # soilmu_l_aoi.shp
        # soilmu_p_aoi.shp
        # soilsf_l_aoi.shp
        # soilsf_p_aoi.shp
        #
        # soilsf_t_aoi.txt is the featdesc file for the tabular import
        #
        ...
        areaSym = "aoi"
        env.workspace = os.path.join(inputFolder, "spatial")
        ...
        if arcpy.Exists(mupolyName):
            ...
            raise MyError, "Error. Missing soil polygon shapefile: " + mupolyName + " in " + os.path.join( inputFolder, "spatial")
        ...
        for sortValue in extentList:
            areaSym = sortValue[0]
            shpPath = os.path.join( inputFolder, "spatial")
            ...
            # Here's where I might consider converting the aoi_a_aoi.shp file.
            #
            ...
            tabularPath = os.path.join( inputFolder,"tabular")
        ...
        if len(mupolyList) > 0:
            ...
            if bGeodatabase:
                ...
                #
                bSpatial = AppendFeatures(outputWS, AOI, mupolyList, mulineList, mupointList, sflineList, sfpointList, featCnt)
                ...
                if bSpatial == True:
                    ...
                    bMD = ImportMDTabular(outputWS, tabularPath, codePage)  # new, import md tables from text files of last survey area

                    if bMD == False:
                        raise MyError, ""

                    # import attribute data from text files in tabular folder
                    bTabular = ImportTabular(outputWS, tabularPath, dbVersion, codePage)
        ...
        PrintMsg(" \nOutput file geodatabase:  " + outputWS, 0)
...
try:
    if __name__ == "__main__":
        inputLayer = arcpy.GetParameterAsText(0)     # thematic soil map shapefile layer
        outputFolder = arcpy.GetParameterAsText(1)    # folder location for output geodatabase

        #dbVersion = 2  # This is the SSURGO version supported by this script and the gSSURGO schema (XML Workspace document)
        # No surveyList variable with this script. Just one single folder will be imported
        desc = arcpy.Describe(inputLayer)
        inputFolder = os.path.dirname(os.path.dirname(desc.catalogPath))
        fcID = os.path.basename(inputFolder)[8:]
        outputWS = os.path.join(outputFolder, "WSS_ThematicMaps_" + fcID + ".gdb")
        aliasName = os.path.basename(inputFolder)[8:]
        AOI = "World"  # hardcode output projection to WGS 1984 Geographic
        useTextFiles = True

        bGood = gSSURGO(inputFolder, outputWS, AOI, aliasName, useTextFiles)
