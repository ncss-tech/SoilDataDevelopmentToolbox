...
#
# Creates Soil Data Viewer-type maps using only the default settings. Designed to run in batch-mode.
            ...
            if dSDV["maplegendkey"] in [3] and dSDV["effectivelogicaldatatype"].lower() in ['choice', 'string', 'vtext']:
                ...
                if "labels" in dLegend and len(dLegend["labels"]) > 0:
                    ...
                    if dLegend["type"] != "1":   # Not NCCPI
                        ...
                        #
                        #PrintMsg(" \nThis is NCCPI with labels: " + str(dLegend["labels"]), 1)
...
## ===================================================================================
def CreateJSONLegend_Old(dLegend, outputTbl, outputValues, ratingField):
    # This does not work for classes that have a lower_value and upper_value
    #
    try:
        # Input dictionary 'dLegend' contains two other dictionaries:
...
def CreateMapLayer(inputLayer, outputTbl, outputLayer, outputLayerFile, outputValues, parameterString, dLayerDefinition):
    ...
    try:
        ...
        PrintMsg("\tSaved to layer file: " + os.path.basename(outputLayerFile) + " \n ", 0)
...
def ReadTable(tbl, flds, wc, level, sql):
    ...
    try:
        ...
        if bVerbose:
            ...
            PrintMsg("Algorithm name: " + dSDV["algorithmname"], 1)

        #if dSDV["algorithmname"].lower() == "percent present":
        if dSDV["algorithmname"].lower() == "percent present":
            with arcpy.da.SearchCursor(tbl, flds,  sql_clause=sql) as cur:
                for rec in cur:
                    rec = list(rec)
                    #PrintMsg(str(rec[-1]), 1)
                    #val = list(rec[1:])
                    val = rec[1:]
                    if rec[-1] == "No":
                        rec[-2] = 0
            ...
            #PrintMsg(" \n" + flds[0] + "; " + ", ".join(flds[1:]), 1)
            #for key, vals in dTbl.items():
            #    PrintMsg(str(key) + ": " + str(vals), 1)
    ...
    try:
        ...
        with arcpy.da.SearchCursor(sdvattTable, "*", where_clause=sql1) as cur:
            ...
            for val in rec:
                ...
                #if not flds[i].lower() in ["maplegendxml", "attributedescription"]:
        ...
        if dSDV["attributetype"].lower() == "interpretation" and dSDV["effectivelogicaldatatype"].lower() in ["float", "integer"]:
            dSDV["notratedphrase"] = ""
...
def CreateRatingTable1S(tblList, sdvTbl, dTbl, initialTbl, dAreasymbols):
    ...
    #
    # How could I modify this function to use only a certain set of areasymbols?
                ...
                for rec in mCur:
                    ...
                    try:
                        ...
                        try:
                            ...
                            try:
                                # Write null values for this component
                                newrec = [dAreasymbols[lkey], mukey, musym, muname]
                                newrec.extend(dMissing["COMPONENT"])
                                #PrintMsg(str(newrec), 1)
                                ocur.insertRow(newrec)
...
## ===================================================================================
def CreateRatingTable2P(tblList, sdvTbl, dComponent, initialTbl):
    # Create table using (mapunit, component) where rating is sum of comppct.
    # This would be used for Hydric Percent Present.
    #
                ...
                for rec in mCur:
                    ...
                    try:
                        ...
                        try:
                            ...
                            for corec in corecs:
                                newrec = [dAreasymbols[lkey], mukey, musym, muname]
                                corec[-1] = corec[-2] # change last two values to comppct (dropping Hydgrp boolean)
                                newrec.extend(corec)
                                #PrintMsg(str(newrec), 0)
                                ocur.insertRow(newrec)
    ...
    try:
        ...
        with arcpy.da.SearchCursor(initialTbl, inFlds, where_clause=whereClause, sql_clause=sqlClause) as cur:
            with arcpy.da.InsertCursor(outputTbl, outFlds) as ocur:
                ...
                # Add final record if there was any data
                if recCnt > 0:
                    newrec = [lastMukey, sumPct, meanVal, areasym]
                    ocur.insertRow(newrec)
...
def AggregateCo_PP_SUM(gdb, sdvAtt, sdvFld, initialTbl):
    ...
    #
    # Added Areasymbol to output
    # Final value in each record should be sum of comppct
    ...
    try:
        ...
        outputTbl = os.path.join(gdb, tblName)
        fldPrecision = max(0, dSDV["attributeprecision"])
        inFlds = ["MUKEY", "AREASYMBOL", "COMPPCT_R", dSDV["attributecolumnname"].upper()]
        outFlds = ["MUKEY", "AREASYMBOL", dSDV["resultcolumnname"].upper()]
        ...
        #PrintMsg(" \n" + dSDV["resultcolumnname"].upper() + " = " + hydric, 1)
        ...
        #PrintMsg(" \nReading " + os.path.basename(initialTbl) + " using fields: " + ", ".join(inFlds), 1)
        #PrintMsg(" \nWriting data to " + os.path.basename(outputTbl) + " using fields: " + ", ".join(outFlds), 1)
        ...
        #with arcpy.da.SearchCursor(initialTbl, inFlds, "", sql_clause=sqlClause) as cur:
            with arcpy.da.InsertCursor(outputTbl, outFlds) as ocur:
                for rec in cur:
                    ...
                    # Sum comppct_r for Hydric mapunits
...
## ===================================================================================
def CreateSoilMap(inputLayer, sdvAtt, top, bot, num=1):
    #
    ...
    try:
        ...
        if dSDV["algorithmname"] == "Minimum or Maximum":
            if tieBreaker == dSDV["tiebreakhighlabel"]:
                PrintMsg(str(num) + ". Creating map of '" + sdvAtt + "' (" + tieBreaker + " value) using " + os.path.basename(gdb), 0)

            else:
                PrintMsg(str(num) + ". Creating map of '" + sdvAtt + "' (" + tieBreaker + " value) using " + os.path.basename(gdb), 0)

        elif dSDV["attributetype"].lower() == "interpretation" and bFuzzy == True:
            PrintMsg(str(num) + ". Creating map of '" + sdvAtt + " (weighted average fuzzy value) using " + os.path.basename(gdb), 0)

        else:
            PrintMsg(str(num) + ". Creating map of '" + sdvAtt + "' (" + dSDV["algorithmname"].lower() + ") using " + os.path.basename(gdb), 0)
        ...
        # 2017-10-01 The following logic all use attributelogicaldatatype. Shouldn't these be
        # effectivelogicaldatatype???
        #
        if bVerbose:
            PrintMsg(" \nAttribute Column Name: " + dSDV["attributecolumnname"], 0)

        if dSDV["effectivelogicaldatatype"].lower() in ['string', 'choice']:
            ...
            raise MyError, "Failed to set dFieldInfo for " + dSDV["columnname"].upper()
        ...
        while rtabphyname != "MAPUNIT":
            ...
            with arcpy.da.SearchCursor(mdTable, mdFlds, where_clause=mdSQL) as cur:
                ...
                for rec in cur:
                    ...
                    if bVerbose:
                        PrintMsg(" \n\tGetting level " + str(level) + " information for " + rtabphyname.upper(), 1)
                    ...
                    if rtabphyname.upper() == dSDV["attributetablename"].upper():
                        ...
                        if rtabphyname == "MAPUNIT" and dSDV["algorithmname"] != "No Aggregation Necessary":
                            ...
                            dMu = ReadTable(rtabphyname, flds, primSQL, level, sql)
                            ...
                            if dSDV["sqlwhereclause"] is not None:
                                if cutOff == 0:
                                    ...
                                    # Skip initial filter for Hydric
                            ...
                            #PrintMsg(" \nPopulating 'dCo' dictionary from component table", 1)
                            #PrintMsg("Reading " + rtabphyname + ": " + dSDV["attributecolumnname"], 1)
                            #PrintMsg(" \nAlgorithm: " + str(dSDV["algorithmname"])  , 1)

                            dCo = ReadTable(rtabphyname, flds, primSQL, level, sql)
                            ...
                            #PrintMsg(" \nhzQuery: " + hzQuery, 1)
                            dHz = ReadTable(rtabphyname, flds, hzQuery, level, sql)
                        ...
                        if rtabphyname == "MAPUNIT":
                            ...
                            dMo = ReadTable(rtabphyname, flds, primSQL, level, sql)
        ...
        if tblList == ['MAPUNIT']:
            ...
            if dSDV["algorithmname"] == "Percent Present":
                if CreateRatingTable2P(tblList, dSDV["attributetablename"].upper(), dCo, initialTbl) == False:
                    raise MyError, ""

            elif CreateRatingTable2(tblList, dSDV["attributetablename"].upper(), dCo, initialTbl) == False:
                raise MyError, ""
            ...
            if CreateRatingTable3(tblList, dSDV["attributetablename"].upper(), dCo, dHz, initialTbl) == False:
                raise MyError, ""
            del dCo, dHz

        elif tblList == ['MAPUNIT', 'COMPONENT', 'CHORIZON', dSDV["attributetablename"].upper()]:
            # COMPONENT, CHORIZON, CHTEXTUREGRP
            if CreateRatingTable3S(tblList, dSDV["attributetablename"].upper(), dCo, dHz, dTbl, initialTbl) == False:
                raise MyError, ""
            del dCo, dHz
            ...
            if dSDV["attributetablename"].upper() == "COINTERP":
                if CreateRatingInterps(tblList, dSDV["attributetablename"].upper(), dCo, dTbl, initialTbl) == False:
                    raise MyError, ""
                del dCo

            else:
                if CreateRatingTable2S(tblList, dSDV["attributetablename"].upper(), dCo, dTbl, initialTbl) == False:
                    raise MyError, ""

        elif tblList == ['MAPUNIT', 'COMPONENT', 'COMONTH', 'COSOILMOIST']:
            if dSDV["attributetablename"].upper() == "COSOILMOIST":

                if CreateSoilMoistureTable(tblList, dSDV["attributetablename"].upper(), dCo, dMo, dTbl, initialTbl, begMo, endMo) == False:
                    raise MyError, ""

                del dMo, dCo # trying to lower memory usage
...
try:
    if __name__ == "__main__":
        ...
        for sdvAtt in sdvAtts:
            num += 1
            bSoilMap = CreateSoilMap(inputLayer, sdvAtt, top, bot, num)
