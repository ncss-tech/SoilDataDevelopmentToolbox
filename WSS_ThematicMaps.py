...
def ReadSummary(summaryFile, mapLayers):
    ...
    try:
        ...
        for choice in mapLayers:
            cnt, mapName = choice.split(".")
            choiceList[int(cnt)] = mapName.strip()

        # There can be duplicate map names, but mapIDs should be unique
...
def ReadLegendNumeric(legendFile, dataType):
    ...
    # IS THIS FUNCTION BEING USED? Need to check working version on my workstation.
    #
    try:
...
## ===================================================================================
def ReadLegendText(legendFile, dataType):
    ...
    try:
        ...
        for rowInFile in csv.reader(open(legendFile, 'rb'), delimiter=',', quotechar='"'):
            if iRow > 0:
                ...
                if iRow == 1 and not dParams["UnitsofMeasure"] is None:
...
try:
    ...
    drawOutlines = arcpy.GetParameter(2)                     # boolean to draw polygon outlines
    fillTransparency = arcpy.GetParameter(3)                 # percent transparency (integer, convert to 255 as opaque)
    ...
    for mapID in mapList:
        ...
        legendFile = os.path.join(thematicPath, "legend" + str(mapID) + ".csv")
        ...
        tblName = dParams["SdvAttributeName"].replace(" ", "_")
        PrintMsg("\t" + mapLayerName, 0)
        ...
        #
        # 2. Legend
        dLegend, colorList = ReadLegendText(legendFile, dataType)
