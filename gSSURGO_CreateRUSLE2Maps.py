# gSSURGO_CreateRUSLE2Maps.py
#
# Creates Soil Data Viewer-type maps for RUSLE2-related properties using only the default settings.
# Designed to run in batch-mode.

# Things to do:
#
# 1. Create soil map layers for: muname, hydgroup, Kf, TFact, Sand, Silt, Clay
# 2. Create merged component-level tabular report
# 3. Create merged map unit-level table (using Merged Rating Table script)
...
def CreateNumericLayer(sdvLyrFile, dLegend, outputValues, classBV, classBL):
    ...
    try:
        ...
        arcpy.AddField_management(dummyFC, dSDV["resultcolumnname"], fType, "", "", fLen)
...
def CreateJSONLegend(dLegend, outputTbl, outputValues, ratingField):
    ...
    try:
        ...
        #bVerbose = True
        arcpy.SetProgressorLabel("Creating soil map legend")
        #bVerbose = True
        ...
        if dLegend["name"] == "Progressive":
            ...
            if dSDV["maplegendkey"] in [3] and dSDV["effectivelogicaldatatype"].lower() in ['choice', 'string', 'vtext']:
                ...
                #PrintMsg(" \ndRatings: " + str(dRatings), 1)
                #PrintMsg(" \ndLegend: " + str(dLegend), 1)
                numItems = len(dLegend["labels"]) # returns a sorted list of legend item numbers
                ...
                raise MyError, "Problem creating legendList1 for: " + dLegend["name"] + ", maplendkey-" +  str(dSDV["maplegendkey"])  # Added the 3 to test for NCCPI. That did not help.
...
def CreateStringLayer(sdvLyrFile, dLegend, outputValues):
    ...
    try:
        ...
        arcpy.AddField_management(dummyFC, dSDV["resultcolumnname"], fType, "", "", fLen, "", "NULLABLE")
...
def CreateMapLayer(inputLayer, outputTbl, outputLayer, outputLayerFile, outputValues, parameterString, dLayerDefinition):
    ...
    try:
        ...
        PrintMsg("\tSaved to layer file: " + os.path.basename(outputLayerFile), 0)

        return finalMapLayer.name
    ...
    try:
        ...
        if dFieldInfo[dSDV["resultcolumnname"].upper()][0] == "TEXT":
            ...
            if finalMapLayer.symbologyType.upper() == "OTHER":
                ...
                PrintMsg("\tManually set 'Unique Values' symbology for this layer using the '" + symField + "' field", 1)
        ...
        PrintMsg("\tSaved soil map to layer file: " + outputLayerFile, 0)
...
def CreateDummyRaster(classBV):
    ...
    try:
        ...
        arcpy.AddField_management(symRaster, dSDV["resultcolumnname"], theType, "", "", dataLen)

        # Populate rating field

        with arcpy.da.UpdateCursor(symRaster, [dSDV["resultcolumnname"]]) as cur:
            i = 0
            for rec in cur:
    ...
    try:
        ...
        with arcpy.da.SearchCursor(tbl, flds, where_clause=wc, sql_clause=sql) as cur:
            for rec in cur:
                ...
                #if bVerbose and rtabphyname == "COSOILMOIST":
...
def GetSDVAtts(gdb, sdvAtt):
    ...
    try:
        ...
        # alter original dSDV["tiebreakrule"] values to make it a boolean. Originally -1, 1.
        # Low will become 0 and High will become 1
        if dSDV["tiebreakrule"] == -1:
            dSDV["tiebreakrule"] = 0

        if dSDV["tiebreaklowlabel"] is None:
            tieList = ["Lower", "Higher"]
            dSDV["tiebreaklowlabel"] = "Lower"
            dSDV["tiebreakhighlabel"] = "Higher"

        else:
          tieList = [dSDV["tiebreaklowlabel"], dSDV["tiebreakhighlabel"]]


        global tieBreaker
        tieBreaker = tieList[dSDV["tiebreakrule"]]
        ...
        if dSDV["algorithmname"] in ["", "No Aggregation Necessary"]:
            dSDV["resultcolumnname"] = dSDV["resultcolumnname"]

        else:
            dSDV["resultcolumnname"] = dSDV["resultcolumnname"] + "_" + dAgg[dSDV["algorithmname"]]
...
def CreateOutputTable(initialTbl, outputTbl, dFieldInfo):
    ...
    try:
        ...
        arcpy.DeleteField_management(outputTbl, "LOCALPHASE")  # probably a better way to handle this
        ...
        arcpy.AddField_management(outputTbl, dSDV["resultcolumnname"], theType, "", "", dataLen)
...
def CreateSoilMoistureTable(tblList, sdvTbl, dComponent, dMonth, dTbl, initialTbl, begMo, endMo):
    ...
    try:
        ...
        if bVerbose:
            PrintMsg(" \nCurrent function : " + sys._getframe().f_code.co_name, 1)
            #PrintMsg(" \ndMissing record list for " + sdvTbl + ": " + str(dMissing[sdvTbl]), 1)
...
def AggregateCo_Limiting(gdb, sdvAtt, sdvFld, initialTbl):
    ...
    # Note! I have some dead code for 'no domain values'. Need to delete if those sectsions are never used.
...
def AggregateCo_MaxMin(gdb, sdvAtt, sdvFld, initialTbl):
    ...
    try:
        ...
        with arcpy.da.InsertCursor(outputTbl, outFlds) as ocur:
            ...
            #
            if dSDV["algorithmname"] == "Minimum or Maximum" and tieBreaker == dSDV["tiebreaklowlabel"]:
                #
                # MIN
                #
                if len(domainValues) > 0:
            ...
            elif dSDV["algorithmname"] == "Minimum or Maximum" and tieBreaker == dSDV["tiebreakhighlabel"]:
                #
                # MAX
                #
                if len(domainValues) > 0:
...
def AggregateCo_DCD(gdb, sdvAtt, sdvFld, initialTbl):
    ...
    try:
        ...
        with arcpy.da.InsertCursor(outputTbl, outFlds) as ocur:
            ...
            # This part is failing for T Factor because the tiebreaklabels are both NULL
            #
...
def UpdateMetadata(outputWS, target, parameterString):
    ...
    #
    try:
        PrintMsg("\tUpdating metadata for the final rating table (" + os.path.join(os.path.basename(outputWS), os.path.basename(target)) + ")...", 0)
        ...
        dMetadata = dict()
        dMetadata["SDV_Title"] = "Rating table for '" + dSDV["attributename"] + "' - " + dSDV["algorithmname"]
...
## ===================================================================================
def CreateSoilMap(inputLayer, sdvAtt, top, bot, mxd, df, num=1):
    #
    # Get these values from sdvattribute table: aggMethod, primCst, secCst, top, bot, begMo, endMo, tieBreaker, bZero, cutOff, bFuzzy, bNulls, sRV
    # Main function
    #
    try:
        ...
        global fc, gdb, muDesc, outputTbl, outputValues, dataType
        ...
        global cutOff, bFuzzy, bNulls, bZero
        cutOff = 0
        bFuzzy = False
        bNulls = False
        bZero = False
        ...
        # automatically override user depth settings to use 0-1cm for Surface ratings
        if sdvAtt.find("Surface") > -1:
            top = 0
            bot = 1

        # aggMethod, primCst, secCst, top, bot, begMo, endMo, tieBreaker, bZero, cutOff, bFuzzy, bNulls, sRV
        #aggMethod = dSDV["algorithmname"]
        begMo = dSDV["beginningmonth"]
        endMo = dSDV["endingmonth"]

        # Print status
        # Need to modify message when type is Interp and bFuzzy is True
        #
        if dSDV["algorithmname"] == "Minimum or Maximum":
            if tieBreaker == dSDV["tiebreakhighlabel"]:
                PrintMsg(" \n" + str(num) + ". Creating map of '" + sdvAtt + "' (" + tieBreaker + " value) using " + os.path.basename(gdb), 0)

            else:
                PrintMsg(" \n" + str(num) + ". Creating map of '" + sdvAtt + "' (" + tieBreaker + " value) using " + os.path.basename(gdb), 0)

        elif dSDV["attributetype"].lower() == "interpretation" and bFuzzy == True:
            PrintMsg(" \n" + str(num) + ". Creating map of '" + sdvAtt + " (weighted average fuzzy value) using " + os.path.basename(gdb), 0)

        else:
            PrintMsg(" \n" + str(num) + ". Creating map of '" + sdvAtt + "' (" + dSDV["algorithmname"].lower() + ") using " + os.path.basename(gdb), 0)
        ...
        dFields["COMPONENT"] = ["MUKEY", "COKEY", "COMPNAME", "LOCALPHASE", "COMPPCT_R"]
        ...
        #
        if dAgg[dSDV["algorithmname"]] != "":
            outputLayer = sdvAtt + " " + dAgg[dSDV["algorithmname"]]
        ...
        if not begMo is None and not endMo is None:
        ...
        while rtabphyname != "MAPUNIT":
            ...
            with arcpy.da.SearchCursor(mdTable, mdFlds, where_clause=mdSQL) as cur:
                ...
                for rec in cur:
                    ...
                    if rtabphyname.upper() == dSDV["attributetablename"].upper():
                        ...
                        if rtabphyname == "MAPUNIT" and dSDV["algorithmname"] != "No Aggregation Necessary":
            ...
            if level > 6:
                raise MyError, "Failed to get table relationships from geodatabase"
        ...
        if int(arcpy.GetCount_management(initialTbl).getOutput(0)) == 0:
            #
            raise MyError, "Query failed to return any results"
        ...
        #
        #PrintMsg(" \ndSDV: " + str(dSDV) + " \n ", 1)
        #PrintMsg(" \n" + dSDV["algorithmname"] + ", " + str(dSDV["complevelattribflag"]) + "; " + str(dSDV["horzlevelattribflag"]) + ", " + str(dSDV["cmonthlevelattribflag"] ), 1)
        ...
        if dSDV["attributetype"] == "Property":
            ...
            if dSDV["mapunitlevelattribflag"] == 1:
                ...
                if dSDV["horzlevelattribflag"] == 0:
                    ...
                    if dSDV["cmonthlevelattribflag"] == 0:
                        ...
                        if dSDV["algorithmname"] == "Dominant Component":
                            ...
                            if len(domainValues) > 0 and dSDV["tiebreakdomainname"] is not None :
                                ...
                                if bVerbose:
                                    PrintMsg(" \n2. aggMethod = " + dSDV["algorithmname"] + " and no domainValues", 1)
                        ...
                        elif dSDV["algorithmname"] == "Weighted Average" and dSDV["attributetype"].lower() == "property":
                        ...
                        elif dSDV["algorithmname"] == "Weighted Average" and SDV["attributetype"].lower() == "intepretation":
                            # Using NCCPI for any numeric component level value?
                            # This doesn't seem to be working for Range Prod 2016-01-28
                            #
                        ...
                        elif dSDV["algorithmname"] == "Percent Present":
                            # This is Hydric?
                        ...
                        if dSDV["resultcolumnname"].startswith("Dep2WatTbl"):
                            ...
                            elif dSDV["algorithmname"] == "Weighted Average":
                            ...
                            if dSDV["algorithmname"] == "Dominant Component":
                              ...
                              outputTbl, outputValues = AggregateCo_Mo_WTA(gdb, sdvAtt, dSDV["attributecolumnname"].upper(), initialTbl)
                                ...
                                raise MyError, "Aggregation method: " + dSDV["algorithmname"] + "; attibute " + dSDV["attributecolumnname"].upper()
                    ...
                    if dSDV["algorithmname"] == "Weighted Average":
                    ...
                    elif dSDV["algorithmname"] == "Dominant Component":
                        ...
                        raise MyError, "'" + dSDV["algorithmname"] + "' aggregation method for " + sdvAtt + " has not been developed"

                else:
                    raise MyError, "Horizon-level '" + dSDV["algorithmname"] + "' aggregation method for " + sdvAtt + " has not been developed"

            else:
                # Should never hit this
                raise MyError, "Unknown depth level '" + dSDV["algorithmname"] + "' aggregation method for " + sdvAtt + " has not been developed"
            ...
            if dSDV["ruledesign"] == 1:
                ...
                elif dSDV["algorithmname"] == "Weighted Average" and dSDV["effectivelogicaldatatype"].lower() == 'float':
                    # Map fuzzy values for interps
                    PrintMsg(" \nNCCPI WTA using Aggregate2_NCCPI function", 1)
                    outputTbl, outputValues = Aggregate2_NCCPI(gdb, sdvAtt, dSDV["attributecolumnname"].upper(), initialTbl)

                elif dSDV["algorithmname"] in ['Least Limiting', 'Most Limiting']:
                ...
                elif bFuzzy or (dSDV["algorithmname"] == "Weighted Average" and dSDV["effectivelogicaldatatype"].lower() == 'float'):
                    # This is NCCPI
                ...
                if dSDV["algorithmname"] == "Dominant Component":
                    ...
                    PrintMsg(dSDV["algorithmname"] + "; " + dSDV["effectivelogicaldatatype"], 1)
    ...
    # End of parameter descriptions
        ...
        global dLayerDefinition  # why am I setting a global here?????
        ...
        if arcpy.Exists(outputTbl):
            ...
            if not dSDV["attributeuom"] is None:
                ...
                parameterString = parameterString + "\r\nAggregation Method: " + dSDV["algorithmname"] + ";  Tiebreak rule: " + tieBreaker
            ...
            if muDesc.dataType.lower() == "featurelayer":
                mapLayerName = CreateMapLayer(inputLayer, outputTbl, outputLayer, outputLayerFile, outputValues, parameterString, dLayerDefinition)
                #PrintMsg(" \nFinished '" + sdvAtt + "' (" + aggMethod.lower() + ") for " + os.path.basename(gdb) + " \n ", 0)

            elif muDesc.dataType.lower() == "rasterlayer":
                mapLayerName = CreateRasterMapLayer(inputLayer, outputTbl, outputLayer, outputLayerFile, outputValues, parameterString)
...
## ===================================================================================
def UpdateSource(gdbinputLayer, sdvLayers, rusle2Tbl):
    # Replace individual rating table joins with SDV_Rusle2 table

    try:
        PrintMsg(" \nUpdating data source for selected layers...", 1)
        numLayers = len(sdvLayers)
        arcpy.ResetProgressor()
        arcpy.SetProgressorLabel("Updating layer data source...")
        arcpy.SetProgressor("step", "Updating data source for selected map layers...", 0, numLayers, 1)

        arcpy.AddJoin_management(inputLayer, "mukey", rusle2Tbl, "mukey")

        for i in range(numLayers):
            sdvLayer = sdvLayers[i]             # 10.1
            if sdvLayer.startswith("'"):
                sdvLayer = sdvLayer[1:-1]

            desc = arcpy.Describe(sdvLayer)

            # make sure each shapefile is stored in the same SDV Temp location
            inDesc = arcpy.Describe(sdvLayer)
            lastShp = inDesc.catalogPath
            thisLoc = os.path.dirname(lastShp)

            # Get arcpy.mapping layer object for the SDV map layer

            #thisMXD = arcpy.mapping.MapDocument("CURRENT")
            #mLayers = arcpy.mapping.ListLayers(thisMXD)

            #for mLayer in mLayers:
            mLayer = arcpy.mapping.ListLayers(mxd, sdvLayer, df)[0]

            # Update the source of the SDV map layer from shapefile to the new geodatabase featureclass
            shapefileName = os.path.basename(mLayer.datasetName)
            PrintMsg(" \t" + mLayer.longName, 0)

            #mLayer.replaceDataSource(thePath, "FILEGDB_WORKSPACE", os.path.basename(outputFC), True)
            mLayer.replaceDataSource(theGDB, "FILEGDB_WORKSPACE", inputLayer, True)

            # Need to get name of joined table and drop the join persisted from Soil Data Viewer
            #
            joinTbl = shapefileName.replace("SdvSoilMapLayer","SdvOutput") + ".txt"
            #PrintMsg(" \nRemoving join " + joinTbl + " from layer " + mLayer.name, 0)

            try:
                # works for ArcGIS 10.0
                arcpy.RemoveJoin_management(mLayer, joinTbl)

            except:
                # works for ArcGIS 10.1
                arcpy.RemoveJoin_management(mLayer)

            theLayerFile = os.path.join(os.path.dirname(theGDB), mLayer.name + ".lyr")
            arcpy.SaveToLayerFile_management(mLayer, theLayerFile, "RELATIVE")
            arcpy.SetProgressorPosition()
...
try:
    if __name__ == "__main__":
        inputLayer = arcpy.GetParameterAsText(0)       # Input mapunit polygon layer (gSSURGO)
        global mxd, df
        mxd = arcpy.mapping.MapDocument("CURRENT")
        df = mxd.activeDataFrame
        ...
        # List of 7 soil property maps to be created
        #sdvAtts = ['Map Unit Name', 'Hydrologic Soil Group','K Factor, Rock Free', 'T Factor', 'Percent Sand', 'Percent Silt', 'Percent Clay']
        sdvAtts = ['Hydrologic Soil Group','K Factor, Rock Free', 'T Factor', 'Percent Sand', 'Percent Silt', 'Percent Clay']
        layerList = []
        originalMaps = list()
        dMapInfo = dict()
        gdb = os.path.dirname(arcpy.Describe(inputLayer).catalogPath)

        for sdvAtt in sdvAtts:
            num += 1
            mapLayerName = CreateSoilMap(inputLayer, sdvAtt, top, bot, mxd, df, num)
            #
            # See if dLayerDefinition gets passed back up to here

            #PrintMsg(" \n" + str(dLayerDefinition), 1)
            if len(dLayerDefinition) > 0:
                dMapInfo[mapLayerName] = dLayerDefinition

            originalMaps.append(mapLayerName)

            # Try saving the SDV_Data table which contains component/horizon data
            # Merge each SDV_Data table to create the Co_RUSLE2 table which will be related to Mupolygon
            # This only works because we are only using a single horizon (top) from each component
            #
            cnt = sdvAtts.index(sdvAtt)
            sdvData = os.path.join(gdb, "SDV_Data")
            coRUSLE2Tbl = os.path.join(gdb, "Co_RUSLE2")

            allFields = arcpy.Describe(sdvData).fields
            fieldList = [fld.name for fld in allFields]
            ratingField = fieldList[-1]
            #PrintMsg(" \n" + sdvAtt + " has fields: " + ", ".join(fieldList), 1)

            if sdvAtt == 'Hydrologic Soil Group':
                joinFields = ["cokey", "comppct_r", ratingField]
                arcpy.CopyRows_management(sdvData,  coRUSLE2Tbl)

            elif sdvAtt  == 'K Factor, Rock Free':
                joinFields = ["hzdept_r", "hzdepb_r", ratingField]
                arcpy.JoinField_management(coRUSLE2Tbl, "cokey", sdvData, "cokey", joinFields)

            else:
                allFields = arcpy.Describe(sdvData).fields
                fieldList = [fld.name for fld in allFields]
                ratingField = fieldList[-1]
                joinFields = [ratingField]
                arcpy.JoinField_management(coRUSLE2Tbl, "cokey", sdvData, "cokey", [ratingField])

            if mapLayerName == "":
                badList.append(sdvAtt)

            else:
                layerList.append(mapLayerName)
        ...
        elif len(layerList) == 6:
            # Got all 6 required RUSLE2 layers
            PrintMsg(" \nMerging layers to create mapunit-level SDV_RUSLE2 table", 0)
            hydroLayer = layerList[0]

            dCompPct = dict()

            fieldList = arcpy.Describe(hydroLayer).fields
            curFields = list()

            for fld in fieldList:
                if fld.baseName.lower() in ["mukey", "comppct_r"]:
                    curFields.append(fld.name)

            hydroTbl = curFields[1].split(".")[0]

            with arcpy.da.SearchCursor(os.path.join(gdb, hydroTbl), ["mukey", "comppct_r"]) as cur:
                for rec in cur:
                    dCompPct[rec[0]] = rec[1]


            # Merge map unit level ratings to a single table: SDV_RUSLE2
            #
            import gSSURGO_MergeRatingTables
            sdvLayers = "; ".join(layerList)
            rusle2Tbl = os.path.join(gdb, "SDV_RUSLE2")

            bMerged = gSSURGO_MergeRatingTables.CreateMergedTable(sdvLayers, rusle2Tbl)  # output table is SDV_RUSLE2 (mapunit ratings)


            #bUpdated = UpdateSource(inputLayer, sdvLayers, rusle2Tbl)

            if 1==1: # Testing...

                # Create a new layer, joined to the merged rating table
                tmpLayer = "tmpLayer"
                tmpLayerFile = os.path.join(env.scratchFolder, "tmpLayerFile.lyr")
                #arcpy.MakeFeatureLayer_management(inputLayer, tmpLayer)
                arcpy.AddJoin_management(inputLayer, "mukey", rusle2Tbl, "mukey")

                # hide duplicate fields from layer
                fieldInfo = arcpy.Describe(inputLayer).fieldInfo
                layerFields = list()
                removeFields = list()

                for i in range(fieldInfo.count):
                    fieldName1 = fieldName = fieldInfo.getfieldname(i)
                    fieldName2 = fieldName1.split(".")[1]

                    if fieldName2 in layerFields:
                        #removeFields.append(fieldName1)
                        fieldInfo.setVisible(i, "HIDDEN")
                        #fieldInfo.removeField(i)

                    else:
                        layerFields.append(fieldName2)

                PrintMsg(" \nCreating new template layer with merged table", 1)
                arcpy.MakeFeatureLayer_management(inputLayer, tmpLayer, "", "", fieldInfo)
                arcpy.SaveToLayerFile_management(tmpLayer, os.path.join(env.scratchFolder, "tmpLayerFile.lyr"))
                arcpy.RemoveJoin_management(inputLayer, os.path.basename(rusle2Tbl))

                #coRUSLE2Tbl = os.path.join(gdb, "Co_RUSLE2")

                arcpy.CreateRelationshipClass_management(coRUSLE2Tbl, inputLayer, "z" + os.path.basename(coRUSLE2Tbl) + "_Mupolygon", "SIMPLE", "< Mupolygon Layer", "> Co_RUSLE2 Table", "NONE", "ONE_TO_MANY", "NONE", "mukey", "mukey", "","")

                # Try to recreate sdvlayers using this new tmpLay
                originalMaps.reverse()

                for layerName in originalMaps:
                    sourceLayer = arcpy.mapping.ListLayers(mxd, layerName, df)[0]
                    sourceDescription = sourceLayer.description
                    targetLayer = arcpy.mapping.Layer(tmpLayerFile)

                    if layerName in dMapInfo:
                        # import symbology from JSON string
                        #PrintMsg(" \nJSON formatting is still not working correctly, need to look into this", 1)
                        dLayerDefinition = dMapInfo[layerName]
                        #PrintMsg(" \n" + str(dLayerDefinition), 1)
                        renderer = dLayerDefinition["drawingInfo"]["renderer"]

                        if "field" in renderer:
                            # Class Breaks renderer
                            ratingField = renderer["field"]
                            ratingField = "SDV_RUSLE2." + ratingField.split(".")[1]
                            dLayerDefinition["drawingInfo"]["renderer"]["field"] = ratingField
                            #

                        elif "field1" in renderer:
                            # Unique Values renderer
                            ratingField = renderer["field1"]
                            ratingField = "SDV_RUSLE2." + ratingField.split(".")[1]
                            dLayerDefinition["drawingInfo"]["renderer"]["field1"] = ratingField

                        else:
                            PrintMsg(" \n" + str(dLayerDefinition), 1)
                            raise MyError, "Problem getting JSON renderer "

                        targetLayer.updateLayerFromJSON(dLayerDefinition)
                        targetLayer.description = sourceDescription
                        arcpy.mapping.RemoveLayer(df, sourceLayer)
                        targetLayer.name = layerName
                        targetLayer.visible = False
                        arcpy.mapping.AddLayer(df, targetLayer)


            PrintMsg(" ", 0)

        else:
            PrintMsg(" \nProblem getting one or more RUSLE2 map layers", 1)
