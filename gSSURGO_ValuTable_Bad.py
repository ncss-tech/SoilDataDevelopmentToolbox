...
def CalcRZDepth(inputDB, outputDB, theCompTable, theMuTable, maxD, dPct, dCR):
    # CalcRZDepth(inputDB, outputDB, theCompTable, theMuTable, 150.0, dPct, dRZRestrictions)
    #
...
def GetCoRestrictions(outputDB, maxD, resList):
    ...
    try:
        ...
        #PrintMsg(" \ndRestrictions: " + str(dRestrictions), 1)
...
## ===================================================================================
def CalcRZAWS(inputDB, outputDB, td, bd, theCompTable, theMuTable, dComp2, maxD, dPct):
    ...
    try:
        ...
        testKeys = dComp2.keys()
        PrintMsg(" \nTesting dComp2 dict which has a first item value of: " + str(dComp2[testKeys[0]]), 1)
            ...
            for rec in inCur:
                ...
                if mjrFlag == "Yes" and cKind != "Miscellaneous area" and cKind is not None:
                    ...
                    try:
                        ...
                        d1, d2, d3, d4, rDepth, restriction = dComp2[cokey]
                        cBot = min(rDepth, bot, maxD)  # 01-05-2015 Added maxD because I found 46 CONUS mapunits with a ROOTZNEMC > 150

                        #if mukey == tmukey:
                        #    PrintMsg("\tRestriction, " + str(mukey) + ", " + str(cokey) + ", " + str(cBot) + ", " + str(restriction), 1)
                        ...
                        if mukey == tmukey:
                            PrintMsg("Restriction not found, " + str(mukey) + ", " + str(cokey) + ", " + str(cBot), 1)
            ...
            if iComp > 0:
                ...
                for corec in coCursor:
                    ...
                    if pctearthmc > 0 and not awc is None:
                        ...
                        if mukey == tmukey:
                            PrintMsg("Mapunit " + mukey + ":" + cokey + "\t" + str(pctearthmc) + " pct_earthy, " + Number_Format(hzT, 2, False) + "cm thick", 1)
                    ...
                    for corec in coCursor:
                        ...
                        if cokey in dComp:
                            ...
                            if sumCompPct > 0:
                                ...
                                if mukey == '1691585':
                                    PrintMsg("\Raw AWS for " + mukey + "|" + cokey + ":\t" + str((aws * adjCompPct)) + "\t, rounded " + str(round((adjCompPct * aws), 2)), 1)
                                    #PrintMsg("\tRounded AWS for " + mukey + "|" + cokey + ":\t" + str(aws), 1)


                                aws = round((adjCompPct * aws), 2) # component rating
                                corec[1] = aws
...
##
    ...
    try:
        ...
        #testKeys = dComp2.keys()
        #PrintMsg(" \nFirst value in dComp2: " + str(dComp2[testKeys[0]]), 1)
        ...
        #if arcpy.Exists(theCompTable):
        #    arcpy.Delete_management(theCompTable)
        PrintMsg("\tSkipping deletion of " + theCompTable, 1)
