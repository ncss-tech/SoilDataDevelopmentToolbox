...
# 2018-09-18 Revert back to coverage to see if the survey boundary cleaner. Yes it does.
#
# 2018-09-21 Currently this script is not handling the GCS output very well. Appears to
# be a problem with the spatial joins for label points to polygon. Maybe I should fine-tune the spatial index for GCS?
#
## ===================================================================================
...
def MakeReportTable(outputGDB, processReport):
    ...
    try:
        ...
        if not arcpy.Exists(reportTbl):
            PrintMsg(" \nCreating new report table: " + reportTbl, 0)
            ...
            arcpy.AddField_management(reportTbl, "POLY_BND", "SHORT", "", "", "")
...
## ===================================================================================
def ProcessSurvey(outputGDB, outputFDS, surveyBoundaryLayer, soilLayer, areasym, method, simpleTol, minAcres, sToday, csName, outputFCName):
    ...
    try:
        # Diagnostic settings
        bCov = True       # flag to use FeatureToCoverage_conversion for polylines
        bCleanup = False    # diagnostic option. remove temporary featureclasses and layers if True.
        ...
        # Try setting a minimum polygon size

        soilSR = arcpy.Describe(soilLayer).spatialReference
        soilSRType = soilSR.type

        if soilSRType == "Geographic":
            soilSRUnits = soilSR.angularUnitName

        else:
            soilSRUnits = soilSR.linearUnitName

        # PrintMsg(" \nSoil layer units: " + soilSRUnits, 1)

        if soilSRUnits == "Degree":
            # set minimum point area to 0.000000001 degrees
            minPointSize = 0.000000001

        elif soilSRUnits == "Meter":
            minPointSize = 0.1
        ...
        # Saving soil mapunit statistics for original source
        #
        ...
        # Adding soil polygons from adjacent survey areas to the selected set. This is to enable boundary cleanup.
        # Ideally we should be collapsing any gaps or overlaps to the 'center-line'. Look at this in the future.
        #
        ...
        PrintMsg("\tConverting original soil polygons to line using " + covTol + " tolerance", 0)

        if bCov:
            # COV
            # Convert soil polygons for this survey and adjacent survey polygons along edge to a coverage
            ...
            # NOT COV
            # Convert soil polygons for this survey and adjacent survey polygons along edge to a line featureclass
            arcpy.PolygonToLine_management(soilLayer, allLinesFC, "IDENTIFY_NEIGHBORS")  # With this new command, output will be 

        # See if setting the output coordinate system and tolerances to match the featuredataset make a difference
        #
        ...
        idList = list()         # list of polygon ids for just this survey area
        dPolygonMukey = dict()  # mukey value for each polygon id

    
        
        if bCov:
            # COV
            polyFlds = ["XX_SOILCOV-ID", "MUKEY"]

        else:                   
            # NO COV
            polyFlds = ["OBJECTID", "MUKEY"]
            
        with arcpy.da.SearchCursor(polyLayer, polyFlds, wc) as cur:
            for rec in cur:
                userid, mukey = rec
        ...
        #
        # Create label points from just the source layer polygons for this survey area
        #
        # ERROR 000072: Cannot process feature with OID 12167. This occurred on a very small overlap polygon in
        # AL129 using GCS WGS1984.
        #
        # Try applying a selection based upon area to eliminate small polygons from FeatureToPoint?
        # polyFlds = arcpy.Describe(polyLayer).fields
        # fldNames = [fld.name for fld in polyFlds]
        # PrintMsg(" \nField list: " + ", ".join(fldNames), 1)
        
        arcpy.SelectLayerByAttribute_management(polyLayer,  "NEW_SELECTION","AREA >= " + str(minPointSize))
        ...
        if bCov:
            # COV
            lineFlds = ["F_ID", "F_LEFTPOLYGON", "F_RIGHTPOLYGON", "MUKEY_L", "MUKEY_R"]

        else:                                     
            # NO COV
            lineFlds = ["F_ID", "LEFT_FID", "RIGHT_FID", "MUKEY_L", "MUKEY_R"]
                                     
        with arcpy.da.UpdateCursor(allLinesFC, lineFlds) as cur:
            ...
            for rec in cur:
                ...
                if not mukeyL is None and mukeyL == mukeyR:
                    # common-soil line created along collapse or survey boundary
        ...
        env.XYTolerance = integrateTol
        # PrintMsg("\tEnvironment XY Tolerance set at " + integrateTol + " for new survey boundary", 1)
        ...
        # Create a featurelayer containing just the soil lines for this survey area
        #
        ...
        sqlSoilLines = "F_ID = -1"  # select only interior soils lines
        soilLines = "SoilLines"
        ...
        # Simplify just the soil lines for this survey area
        #
        ...
        PrintMsg("\tAssembling simplified featureclass " + os.path.basename(outputFC) + " with an XYTolerance of " + str(env.XYTolerance), 0)

        theSoilLabels = "SoilLabels"

        arcpy.MakeFeatureLayer_management(soilLabels, theSoilLabels, wc)  # Shouldn't need wildcard because soilLabels featureclass is for this survey only.
        ...
        # Convert survey boundary lines and soil lines back to polygon featureclass
        #
        ...
        # This seems to get most of the polygons
        #
        ...
        # NEW TEST. CONVERT BOUNDARY LINES TO POLYGON, GETCOUNT
        testBndFC = os.path.join(env.scratchGDB, "TestBnd_" + areasym)
        arcpy.FeatureToPolygon_management(soilBnd, testBndFC)
        bndPolyCnt = int(arcpy.GetCount_management(testBndFC).getOutput(0))

        if bndPolyCnt > 1:
            PrintMsg("\tBoundary lines form " + str(bndPolyCnt) + " polygons", 1)
        ...
        # if bndCnt == 0:
            ...
            PrintMsg("\tEliminating " + Number_Format(len(elimList), 0, True) + " small polygons less than " + str(minAcres) + " acres in size", 1)
        ...
        if len(centroidList) > 0:
            ...
            for centroidPnt in centroidList:
                ...
                if srcCnt == 0:
                    # Second attempt to get the source data attribute
                    # IA001 had 3 polygons that failed to select when out output coordinate system is geographic
                    # Why would it make any difference what coordinate system the output feature class is in different?
                    #
                    # Try setting the SearchCursor of the source to use the SR of the output featureclass. Get the
                    # centroids in the same SR as the output featureclass.
                    #
                    PrintMsg("\tSecond attempt to get source attributes using SelectLayerByLocation", 1)
                    arcpy.SelectLayerByLocation_management(soilLayer, "INTERSECT", centroidPnt, "", "NEW_SELECTION")

                if srcCnt > 0:

                    # Use the same coordinates to select the output polygon that is missing data. 
                    ...
                    missedShot += 1
                    PrintMsg(str(missedShot) + ", " + str(centroidPnt.centroid.X) + ", " + str(centroidPnt.centroid.Y), 1)
        ...
        fieldNames = ["AREASYMBOL", "LAYER_NAME", "RUN_DATE", "POLY_CNT1", "POLY_CNT2", "POLY_DIFF", "POLY_ELIM", "NO_ATTS", "POLY_BND", "VERT_CNT1", "VERT_CNT2", "VERT_DIFF", "VERT_PCT", "SETTINGS", "CS_NAME"]
                
        vertexChng = vertexCnt1 - vertexCnt2
        vertPct = round((vertexChng * 100.0 / vertexCnt2), 1)

        rec = [areasym, outputFCName, sToday, polyCnt1, polyCnt2, polyDiff, len(elimList), noAtt, bndPolyCnt, vertexCnt1, vertexCnt2, vertexChng, vertPct, settings, csName]
        ...
        if bCleanup:
            ...
            layerList = [tmpFC, allLinesFC, bndLayer, simpleSoils, soilLabels, bndLayer, soilBnd, outputCov, joinFC, testBndFC]
        ...
        del tmpFC, allLinesFC, simpleSoils, soilLabels, bndLayer, soilBnd, outputCov, joinFC, testBndFC
...
try:
    ...
    minAcres = arcpy.GetParameter(7)         # polygons less than this many acres will be targeted for elimination
    bOverwrite = arcpy.GetParameter(8)
    bCleanUp = arcpy.GetParameter(9)
    ...
    for areasym in surveyList:  # Process in alphabetical order
        ...
        if bOverwrite == False and arcpy.Exists(os.path.join(outputFDS, outputFCName)):
            ...
            bDone = ProcessSurvey(outputGDB, outputFDS, surveyBoundaryLayer, soilLayer, areasym, method, simpleTol, minAcres, sToday, fdsSR.name, outputFCName)
