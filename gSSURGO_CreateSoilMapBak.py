...
def CreateJSONLegend(dLegend, outputTbl, outputValues, ratingField, sdvAtt, bFuzzy):
    ...
    try:
        ...
        if dLegend["name"] == "Random" and len(dLegend["colors"]) == 0 and len(dLegend["labels"]) == 0:
            ...
            #if len(outputValues) > 0:
            #    PrintMsg(" \nStopped here for the night", 1)

            # New code 2018-08-02. Trying to handle unpublished interps with no symbology settings
            if dSDV["attributetype"].lower() == "interpretation" and len(outputValues) > 0:
                # try running UniqueValuesJSON from here
                legendList = list()
                numItems = len(outputValues)
                ...
                for i in range(0, numItems):
                    try:
                        ...
                        rgb = rgbColors[i]
                        rating = outputValues[i]
                        
                        if str(rating).lower() == "not rated" or str(rating).lower() == "none":
                            rgb = dkGray
                            
                        legendLabel = str(rating)
                        legendList.append([rating, legendLabel, rgb])
                        # PrintMsg("\tRandom legend:  " + str(i) + ". '" + str(rating) + "',  '" + str(legendLabel) + "',   rgb: " + str(rgb), 1)
        ...
        # Let's try maplegendkey as the driving force...
        PrintMsg(" \nLooking at maplegendkey value for symbology clues...", 1)
...
def UniqueValuesJSON(legendList, outputTbl, ratingField):
    ...
    try:
        ...
        if dSDV["attributetype"] == "Interpretation":
            ...
            legendItems["symbol"] = symbol
            #PrintMsg(" \nAdded dark gray symbol for 'Not rated'", 1)
            uniqueValueInfos.append(legendItems)
...
def CreateMapLayer(inputLayer, outputTbl, outputLayer, outputLayerFile, outputValues, parameterString, creditsString, dLayerDefinition, bFuzzy):
    ...
    try:
        ...
        if dLegend["name"] == "Random" and len(dLegend["labels"]) == 0:
            #
            # 'New Interps' are passing through through this section of code. Losing ability to set 'Not rated' to gray.
            #
            #
            #
            ...
            # PrintMsg(" \nUpdating layer symbology using " + sdvLyrFile, 1)
            ...
            if finalMapLayer.symbologyType.upper() == 'UNIQUE_VALUES':
                ...
                PrintMsg(" \nfinalMapLayer.symbology = " + finalMapLayer.symbologyType.upper(), 1)
    ...
    try:
        ...
        with arcpy.da.SearchCursor(tbl, flds, where_clause=wc, sql_clause=sql) as cur:
            for rec in cur:
                ...
                #if tbl == "CORESTRICTIONS":
                #    PrintMsg(" \tCR: " + rec[0] + "  " + str(val), 1)
    ...
    try:
        ...
        if dSDV["attributetype"].lower() == "interpretation" and (dSDV["effectivelogicaldatatype"].lower() == "float" or bFuzzy == True):
            ...
            if dSDV["attributetype"].lower() == "interpretation" and dSDV["attributelogicaldatatype"].lower() == "string" and dSDV["effectivelogicaldatatype"].lower() == "float":
                ...
                # New issue. Skip this section for interps that have not been published.
...
## ===================================================================================
def UpdateMetadata(outputWS, target, parameterString, creditsString, aggMethod, sdvAtt):
    #
...
def CreateSoilMap(inputLayer, sdvAtt, aggMethod, primCst, secCst, top, bot, begMo, endMo, tieBreaker, bZero, cutOff, bFuzzy, bNulls, sRV):
    ...
    try:
        ...
        #PrintMsg(" \nChecking dLegend to see if existence of 'labels' is a good check for unpublished interps", 1)
        #x = dLegend["labels"]
        #PrintMsg(str(dLegend["labels"]), 1)
        #if dSDV["attributetype"].lower() == "interpretation" and len(dLegend["labels"]) == 0:
        #    # This is an unpublished interpretation
        #    dSDV["maplegendkey"] = 5
        ...
        #else:
        #    PrintMsg(" \noutputValues: " + str(outputValues), 1)  # This is a problem only in Batch mode. Why?

        #if not bVerbose:  # Need to keep output table for reports
        ...
        if outputValues != [-999999999, 999999999]:
            ...
            if dLegend["name"] != "Random":  # trying to get raster to work for Hydric
                ...
                if bVerbose:
                    PrintMsg(" \nOn the new Capability Subclass track", 1)
            ...
            if arcpy.Exists(outputTbl):
                ...
                bMetadata = UpdateMetadata(gdb, outputTbl, parameterString, creditsString, aggMethod, sdvAtt)
