# InterpDataMapping2.py
#
# Steve Peaslee, National Soil Survey Center
#
# Creates tables and maps for a selected interpretation in the SDVATTRIBUTE table.
# Maps the weighted fuzzy value for each rating reason.
#
# Adapted from InterpDataMapping.py
#
# Last revision 2016-12-11
#
# ALL: all components
...
# Currently I can map the fuzzy values for each crop type, but not the individual rating reasons.
# Need to compare with the VALU1 table.
#
# 2015-10-07 - Set visibility to False for each new layer and set overwriteOutput to True
#
# 2017-01-10 - Modified output table and output map layers to show component-weighted fuzzy values * 100 (integer)
# The integer values make it easier to read the map legend and display the values.
#
## ===================================================================================
...
def MapRatingReasons(theDB, theMapunitLayer, theCompTable, cFlds, outputField, dRatingClasses):
    ...
    try:
        ...
        + "\r\nMapped rating values are the product of component percent and fuzzy values * 100 (as integer)" \
        + "\r\nGreen indicates a degree restriction level. Red indicates a higher degree of restriction."
        ...
        #PrintMsg(" \nCreating relationshipclass: " + rcName, 0)

        if arcpy.Exists(rcName):
            arcpy.Delete_management(rcName)
        ...
        with arcpy.da.SearchCursor(theCompTable, cFlds) as cCursor:
            ...
            # 2017-01-09 I think this section is where I would need to change
            # the Rating Reason maps to use a weighted fuzzy value instead of comppct.
        ...
        if bPivot == False:
            raise MyError, "Failed to add columns to " + theMuTable
        ...
        #PrintMsg(" \ndRatingClasses: " + str(dRatingClasses), 1)
        ...
        with arcpy.da.SearchCursor(qTable, qFields, sql_clause=sqlClause) as qCursor:
            ...
            for qRec in qCursor:
                ...
                if ruleDepth == 0:
                    ...
                    cCursor.insertRow(cRec)
                    #PrintMsg("\t" + theMukey + ", " + compName + ", " + ratingClass + ", " + str(fuzzyValue), 1)
