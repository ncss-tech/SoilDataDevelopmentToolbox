...
#
# 2019-02-20 Created soil map layer as an SQLite View. This seems to work even though the tables are not recognized.
#
...
def CreateInitialTable(gdb, allFields, dFieldInfo, finalQuery, dbConn, liteCur):
    ...
    try:
        ...
        # Need logic to automatically switch between an in-memory database or the output database on disk,
        # based upon size of database.
        # Problem. ArcGIS does not seem to recognize :memory" databases created using sqlite3 library.

        dbSize = "large"

        if dbSize == "large":
            # Create sdv_data table in the input sqlite database
            ...
            queryCreateTable1 = "CREATE TABLE " + initialTbl + " ( "
            queryCreateTable = queryCreateTable1 + queryCreateTable
            #PrintMsg(" \n" + queryCreateTable, 1)
            liteCur.execute(queryCreateTable)
            dbConn.commit()
            
            # Write raw data to the input database
            #PrintMsg(" \nInsert begins...", 0)
            ...
            #
            # Problem with this :memory: method.  Don't know how to access the attached database with a cursor.
            #
            # Attach in-memory database to database on disk.
            # Use in-memory sqlite database to write raw data

            # set initialTbl location to dbMem
            initialTbl = "dbMem." + initialTbl

            queryDrop = "DROP TABLE IF EXISTS " + initialTbl + ";"
            queryCreateTable1 = "CREATE TABLE " + initialTbl + " ( "

            # Try attaching the memory database to the on-disk SQLite database
            queryAttach = "ATTACH DATABASE ':memory:' AS dbMem"
            liteCur.execute(queryAttach)
            dbConn.commit()
...
def CreateOutputTable(initialTbl, outputTbl, dFieldInfo):
    ...
    try:
        ...
        #PrintMsg(" \nCreating table " + outputTbl + " using template: " + initialTbl, 1)
        #arcpy.CreateTable_management(os.path.dirname(outputTbl), os.path.basename(outputTbl), initialTbl)
        #PrintMsg(" \nUsing " + os.path.dirname(outputTbl) + "; " + os.path.basename(outputTbl), 1)
        #PrintMsg("\tCurrent workspace: " + env.workspace, 1)
        # Create connection to database
        ...
        fldList = [fld.name.lower() for fld in arcpy.Describe(initialTbl).fields]

        #fieldName = dSDV[resultcolumn]
        theType = dFieldInfo[dSDV["resultcolumnname"].lower()][0]
        ...
        dataLen = dFieldInfo[dSDV["resultcolumnname"].lower()][1]

        #queryCreateTable = "CREATE TABLE " + os.path.basename(outputTbl) + " ( OBJECTID INTEGER NOT NULL PRIMARY KEY, mukey text, areasymbol text, "
        queryCreateTable = "CREATE TABLE " + os.path.basename(outputTbl) + " ( objectid integer primary key autoincrement, mukey text (30), areasymbol text (20), "

        if "comppct_r" in fldList:
            queryCreateTable += "comppct_r integer, "

        if theType == "text":
            queryCreateTable += dSDV["resultcolumnname"].lower() + " " + theType + "(" + str(dataLen)  + ") " + "); "

        else:
            queryCreateTable += dSDV["resultcolumnname"].lower() + " " + theType + "); "
...
def CreateSoilMap(inputLayer, sdvAtt, aggMethod, primCst, secCst, top, bot, begMo, endMo, tieBreaker, bZero, cutOff, bFuzzy, bNulls, sRV):
    ...
    try:
        ...
        if not version[0:4] in ["10.3", "10.4", "10.5", "10.6"]:
