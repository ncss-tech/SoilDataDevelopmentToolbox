...
def AddNewFields(outputShp, columnNames, columnInfo):
    # Create the empty output table that will contain the data returned by the Tabular post.rest request
    #
    ...
    #
    try:
        # Dictionary: Convert SQL Server data types to ESRI File Geodatabase
        dType = dict()
...
,(SELECT CASE when min(resdept_r) is null then 200 else min(resdept_r) END from component left outer join corestrictions on component.cokey = corestrictions.cokey where component.cokey = c.cokey and reskind is not null) as restrictiodepth
...
SELECT ((0.065 + (0.0456 * slope_r) + (0.006541*Power(slope_r, 2)))*Power((slopelenusle_r / 22.1) , k.mexp_h)) AS LS_r, k.cokey, k.mukey
INTO #LS
FROM #cdsi
...
## ===================================================================================
def FormAttributeQueryCDSI(mukeys):
    #
    # Given a simplified polygon layer, use vertices to form the spatial query for a Tabular request
    # Coordinates are GCS WGS1984 and format is WKT.
    # Returns spatial query (string) and clipPolygon (geometry)
...
def AttributeRequest(theURL, mukeyList, outputShp, ratingField):
    ...
    try:
        ...
        PrintMsg(" \nOutput to featureclass: " + outputShp)
        ...
        if len(maxminValues) > 1:
            ...
            raise MyError, "No data for " + ratingField + " found"
...
def FormSpatialQuery(theAOI):
    ...
    try:
        ...
        wktAOI = wkt.replace("MULTIPOLYGON (", "POLYGON ")[:-1]

        sdaQuery = """
 ~DeclareGeometry(@aoi)~
 select @aoi = geometry::STPolyFromText('""" + wktAOI + """', 4326)
...
def RunSpatialQuery(theURL, spatialQuery, outputShp, clipPolygon, showStatus):
    ...
    try:
        ...
        PrintMsg(" \n\tProcessing spatial request using " + url + " in " + dRequest["FORMAT"] + " output", 1)
        PrintMsg(" \n" + spatialQuery, 1)
