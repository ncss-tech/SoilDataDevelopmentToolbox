# SSURGO_BatchDownloadFromDev.py
#
# Download SSURGO data from Dev cache. Probably a one time use.
#
# Using a hard list of zipfile names to download
#
## ===================================================================================
...
## ===================================================================================
def GetDownload(zipName, areaSym, surveyDate):
    ...
    baseURL = "https://soilsdashboard-dev.dev.sc.egov.usda.gov/DataManager/WSSFileShare?dir=DownloadSoilsData/PreCache/SSA/"
    ...
    try:
        ...
        #PrintMsg(" \nRequest code: " + str(request.code), 1)  # It think code of 200 is good
        ...
        #PrintMsg(" \nOpening new zipfile " + local_zip, 1)
...
## ===================================================================================
def ProcessSurvey(outputFolder, zipName, bImport, bRemoveTXT, iGet, iTotal):
    ...
    try:
        ...
        areaSym = zipName[8:13]

        # get date string
        surveyDate = zipName[15:25].replace("-", "")
        PrintMsg(surveyDate, 1)
        ...
        if bNewer:
            ...
            #
            PrintMsg(" \nProcessing survey " + areaSym + " (" + str(iGet) + " of " + str(iTotal) + ")", 0)

            # First attempt to download zip file
            bDownloaded = GetDownload(zipName, areaSym, surveyDate)
            ...
            bZip = UnzipDownload(outputFolder, newFolder, "", zipName)
...
import arcpy, sys, os, locale, string, traceback, shutil, zipfile, subprocess, glob, socket, csv, re, httplib
...
try:
    ...
    zipFileList = os.path.join(os.path.dirname(sys.argv[0]), "DevZipfileList.txt")
    zipList = list()

    fh = open(zipFileList, "r")
    for line in fh:
    ...
    PrintMsg(" \n" + str(len(zipList)) + " soil survey(s) selected for Web Soil Survey download", 0)
    ...
    #    surveyInfo = survey.split(",")
        ...
        asDict[areaSym] = zipName

    #asList.sort()
    iTotal = len(zipList)
    arcpy.SetProgressor("step", "Downloading SSURGO data...",  0, iTotal, 1)
    ...
    for zipFile in zipList:
        ...
        arcpy.SetProgressorLabel("Downloading survey " + areaSym + " from Web Soil Survey  (number " + str(iGet) + " of " + str(len(zipList)) + " total)")
        bProcessed = ProcessSurvey(outputFolder, zipFile, False, False, iGet, iTotal)
